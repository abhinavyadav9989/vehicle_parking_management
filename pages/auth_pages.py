"""Simplified authentication UI redesigned to match provided mockup."""
...
HEADER_COLOR = "#1e3a8a"  # blue banner
REGISTER_ACTIVE = "#f59e0b"  # orange
...
class AuthPage:
    """Authentication card with register/login views matching supplied design."""
    ...
    def __init__(self, parent, role, auth_service, on_back, on_success):
        ...
        ctk.CTkLabel(
            header_bar,
            text=f"Welcome {self.role_display}",
        ...
        self.register_button = ctk.CTkButton(
            ...
            height=40,
            corner_radius=6,
            command=lambda: self._show_view("register"),
        )
        ...
        self.login_button = ctk.CTkButton(
            ...
            height=40,
            corner_radius=6,
            fg_color=TOGGLE_INACTIVE_FG,
            text_color="#111827",
            hover_color="#f3f4f6",
            border_width=1,
            border_color=TOGGLE_INACTIVE_BORDER,
            command=lambda: self._show_view("login"),
        )
        self.login_button.grid(row=0, column=1, padx=(4, 0))

        self.views = {
            "register": self._build_register_view(card),
            "login": self._build_login_view(card),
        }
        self.current_view: str | None = None

        # Toolbar with back button aligned to left under toggles
    ...
    def _build_register_view(self, parent: ctk.CTkFrame) -> ctk.CTkFrame:
        ...
        for idx, (label_text, attr_name) in enumerate(fields):
            ctk.CTkLabel(
                ...
                text_color="#111827",
            ).grid(row=idx, column=0, sticky="e", padx=(22, 18), pady=8)
    ...
    def _build_login_view(self, parent: ctk.CTkFrame) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(
            ...
            font=CTkFont(size=14),
            text_color="#111827",
        ).grid(row=0, column=0, sticky="e", padx=(22, 18), pady=8)
        self.login_email_entry = ctk.CTkEntry(frame, height=36, width=270)
        ...
        ctk.CTkLabel(
            ...
            font=CTkFont(size=14),
            text_color="#111827",
        ).grid(row=1, column=0, sticky="e", padx=(22, 18), pady=8)
        self.login_password_entry = ctk.CTkEntry(frame, height=36, width=270, show="*")
